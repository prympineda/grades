...
<h1> Add Performance Task : {{ ucwords($section->grade_level->name) }}-{{ ucwords($section->name) }} -
...
<form action="{{ route('post_add_performance_task') }}" method="POST" autocomplete="off">
    <div class="form-group">
        <input type="number" name="total" id="total" max=100
            onkeypress='return event.charCode >= 48 && event.charCode <= 57' placeholder="Total" />
    </div>
    <div class="form-group">
