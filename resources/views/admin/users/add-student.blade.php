            ...
            <div class="card-body">
                ...
                <form action="{{ route('post_add_student') }}" method="POST" autocomplete="off">
                    <div class="form-group row">
                        <label class="col-md-4 col-form-label text-md-right">Grade Level & Section </label>
                        <div class="col-md-6">
                            <select name="section" id="" class="form-control" required>
                                <option value="">Select Grade Level &amp; Section</option>
                                @foreach($sections as $s)
                                <option value="{{ $s->id }}">{{ ucwords($s->grade_level->name) }} -
                                    {{ ucwords($s->name) }}</option>
                                @endforeach
                                @if(count($sections) == 0)
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                            <input type="text" class="form-control" onkeypress="return isNumberKey(event);"
                                name="mobile" maxlength="12" value="{{ old('mobile') }}" placeholder="Mobile Number"
                                required>
                        </div>
                    </div>
